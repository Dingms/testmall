  ...
  <div class="cart-container container">
    ...
    <section v-else-if="itemList.length" ref="wrapper">
      <div>
        <div class="cart-title">
          <van-checkbox checked-color="#e1251b" icon-size="20px" v-model="checkedAll"
            @click="checkedChange"></van-checkbox>
          <span>商品</span>
        </div>
        <ul>
          <li v-for="item in itemList" :key="item.id">
            <div class="checkbox">
              <van-checkbox v-model="item.checked" checked-color="#e1251b" icon-size="20px"
                @click="checkedHandler"></van-checkbox>
            </div>
            <div>
            ...
            <div class="goods">
              <div class="goods_title">
                ...
                <van-icon name="delete-o" size="20px" @click="delCartItem(item.id)"/>
              </div>
              <div class="goods-price">
                <p>￥{{ item.price }}</p>
              </div>
              <van-stepper v-model="item.num" integer input-width="30px" button-size="24px" @change="changeNumHandler(item.id,item.num)"/>
            </div>
          </li>
        </ul>
      </div>
    ...
    <footer v-if="itemList.length && loginStatus">
      <ul class="cart-footer">
        <li class="radio">
          <van-checkbox v-model="checkedAll" checked-color="#e1251b" icon-size="20px"
            @click="checkedChange"></van-checkbox>
        </li>
        <li class="total">
...
const checkedChange = () => {
  itemList.value = itemList.value.map(item => ({ ...item, checked: checkedAll.value }))
  cartChange()
}
